...
def compute_x(df, lm_intercept, total_point, maxi_point, mini_point=0):
    ...
    score = np.linspace(
        coef * mini_overallpoint + mini_score,
        coef * maxi_overallpoint + mini_score,
        500,
    )
    prob = 1 / (1 + np.exp(-score))
...
def grid_connect(
    ax1, ax2, xticks, prop_lines={"lw": 0.5, "color": "b", "linestyle": "-."}, **kwargs
):
...
def nomogram(
    ...
    # change for compatability (was ["main_feature"])
    ...
    mini_overallpoint, maxi_overallpoint, x_point, prob = compute_x(
        df=df, lm_intercept=lm_intercept, total_point=total_point, maxi_point=maxi_point
    )

    fig = plt.figure(
        figsize=(fig_width, single_height * (num + 2 + 7)), dpi=dpi
    )  # 创建画布
    ...
    for i in range(0, len(feature)):
        ...
        if d.shape[0] > 1:
            ...
            # add .item() for compatability
            maxi = float(d["max"].item())
            mini = float(d["min"].item())
            point = d["point"].values[0]
            range_ = maxi - mini

            xticks, xticklabels, position = generate_xtick(
                range_,
                type_=list(set(d["type"].values))[0],
                mini=mini,
                maxi=maxi,
                point=point,
                total_point=total_point,
            ...
            if d["coef"].values < 0:  # type: ignore
    ...
    xticks, xticklabels, position = generate_xtick(
        maxi_overallpoint - mini_overallpoint,
        type_="continuous",
